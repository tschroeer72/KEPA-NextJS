...
export async function GET(request: Request) {
  ...
  try {
    ...
    const allResults = await prisma.tblSpielBlitztunier.findMany({
      where: {
        tblSpieltag: {
    ...
    // Dubletten nach ID entfernen (falls Prisma-Joins oder DB-Einträge doppelt kommen)
    const rawResults = Array.from(new Map(allResults.map(r => [r.ID, r])).values());

    // Letztes Spieldatum ermitteln
    const lastDate = rawResults.length > 0 ? rawResults.reduce((max, curr) => {
      const currDate = curr.tblSpieltag.Spieltag;
      return currDate > max ? currDate : max;
    }, rawResults[0].tblSpieltag.Spieltag) : new Date();
    ...
    rawResults.forEach((item) => {
      ...
      if (!teilnehmerMap.has(item.SpielerID1)) {
        const p = item.PunkteSpieler1;
        const gp = item.PunkteSpieler2;
        teilnehmerMap.set(item.SpielerID1, {
          ...
          Anzahl: 1,
          Gew: item.PunkteSpieler1 > item.PunkteSpieler2 ? 1 : 0,
          Unent: item.PunkteSpieler1 === item.PunkteSpieler2 ? 1 : 0,
          Verl: item.PunkteSpieler1 < item.PunkteSpieler2 ? 1 : 0,
        });
      } else {
        ...
        t.Anzahl += 1;
        if (item.PunkteSpieler1 > item.PunkteSpieler2) t.Gew += 1;
        else if (item.PunkteSpieler1 === item.PunkteSpieler2) t.Unent += 1;
        else t.Verl += 1;
      }

      // Spieler 2
      if (!teilnehmerMap.has(item.SpielerID2)) {
        const p = item.PunkteSpieler2;
        const gp = item.PunkteSpieler1;
        teilnehmerMap.set(item.SpielerID2, {
          ...
          Anzahl: 1,
          Gew: item.PunkteSpieler2 > item.PunkteSpieler1 ? 1 : 0,
          Unent: item.PunkteSpieler2 === item.PunkteSpieler1 ? 1 : 0,
          Verl: item.PunkteSpieler2 < item.PunkteSpieler1 ? 1 : 0,
        });
      } else {
        ...
        t.Anzahl += 1;
        if (item.PunkteSpieler2 > item.PunkteSpieler1) t.Gew += 1;
        else if (item.PunkteSpieler2 === item.PunkteSpieler1) t.Unent += 1;
        else t.Verl += 1;
      }
    });
    ...
    if (showSpielergebnisse) {
      ...
      meisterschaft.tblSpieltag.forEach((st) => {
        ...
        // Initialisiere alle Meisterschaftsteilnehmer für diesen Spieltag
        meisterschaft.tblTeilnehmer.forEach(t => {
          // Name suchen: Zuerst in den aktuellen Spieltag schauen (effizienter)
          let mitglied = st.tblSpielBlitztunier
            .find(sm => sm.SpielerID1 === t.SpielerID || sm.SpielerID2 === t.SpielerID);
          
          // Wenn nicht gefunden, in der gesamten Meisterschaft suchen
          if (!mitglied) {
            mitglied = meisterschaft.tblSpieltag.flatMap(s => s.tblSpielBlitztunier)
              .find(sm => sm.SpielerID1 === t.SpielerID || sm.SpielerID2 === t.SpielerID);
          }
          ...
          if (mitglied) {
            ...
            name = m ? (m.Spitzname || `${m.Vorname} ${m.Nachname}`) : "";
          }
        ...
        // Blitztunier-Spiele prüfen ob Spieler teilgenommen hat
        st.tblSpielBlitztunier.forEach(sm => {
          const s1 = spielerDaten.get(sm.SpielerID1);
          if (s1) s1.hatGespielt = true;
          const s2 = spielerDaten.get(sm.SpielerID2);
          if (s2) s2.hatGespielt = true;
        });
        ...
        aktiveSpieler.forEach(s => {
          ...
          xRow += 1.0;
          drawBox(xRow, yPos, 6.0, 0.6, s.name, 10, false, "left");
          xRow += 6.0;
          drawBox(xRow, yPos, 1.5, 0.6, s.neuner.toString(), 10);
          xRow += 1.5;
    ...
    if (showTabelle) {
      ...
      doc.setFont("helvetica", "bold");
      doc.setFontSize(18);
      const tableWidth = 1.5 + 5 + 4 * 1.5 + 2; // Platz, Name, Spiele(4), Punkte
        ...
        rawResults.forEach(r => {
          if (r.SpielerID1 === lstTeilnehmer[i].SpielerID) {
            if (r.HinR_ckrunde === 0) {
              ...
              sumHin += r.PunkteSpieler1;
            } else {
              sumRueck += r.PunkteSpieler1;
            }
          } else if (r.SpielerID2 === lstTeilnehmer[i].SpielerID) {
            if (r.HinR_ckrunde === 0) {
              if (getNurHinrunde(r.SpielerID1)) return;
              sumHin += r.PunkteSpieler2;
            } else {
              sumRueck += r.PunkteSpieler2;
            }
          }
        });
